...
if [[ -z $1 ]]
    then
        ...
        if [[ ! $1 =~ ^[0-9]+$ ]]
      then
        RESULT=$($PSQL "SELECT * FROM properties JOIN elements USING(atomic_number) JOIN types USING(type_id) WHERE name LIKE '$1' OR symbol LIKE '$1' ORDER BY atomic_number")
